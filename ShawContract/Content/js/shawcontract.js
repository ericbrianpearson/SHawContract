(function () {
    ...
    try {
        document.getElementById('product_color_label_switch').addEventListener("change", onColorLabelChange);
    }
    catch (error) { }
    ...
    function onDocumentLoaded() {
        try {
            ...
            $('.save-to-board').click(function (e) {
                ...
                alert('save to board called with values "Product: ' + productNumber + '" and "Color: ' + colorNumber + '"');
            });
            ...
            $('.btn-maximize').click(function (e) {
                ...
                if (parent) {
                    $('.swappable').removeClass('maximized minimized col-sm-12 col-sm-6 order-6')
                    parent.addClass('col-sm-12 maximized');
                    $('.swappable').not(parent).addClass('col-sm-6 minimized order-6');
                }
            });
    ...
    function onColorLabelChange() {
        var isChecked = document.getElementById('product_color_label_switch').checked;
        if (isChecked) {
            document.getElementById('product_color_picker').classList.add('show-labels');
            $('[data-toggle="tooltip"]').tooltip('dispose');
        }
        else {
            document.getElementById('product_color_picker').classList.remove('show-labels');
            $('[data-toggle="tooltip"]').tooltip();
        }
    }
    ...
    function updateProductDetailLoadProgress(intPercentComplete) {
        ...
        if (intPercentComplete >= 100) { productDetailLoadComplete(); }
    }
    ...
    function fakeProgress() {
        ...
        setTimeout(function () { updateProductDetailLoadProgress(35); }, 500);
        setTimeout(function () { updateProductDetailLoadProgress(45); }, 1000);
        setTimeout(function () { updateProductDetailLoadProgress(65); }, 1500);
        setTimeout(function () { updateProductDetailLoadProgress(85); }, 2000);
        setTimeout(function () { updateProductDetailLoadProgress(95); }, 2500);
        setTimeout(function () { updateProductDetailLoadProgress(100); }, 3100);
    }
})();